...
public class RestockerTourProcessResource {
    ...
    public RestockerTourProcessResource(RuntimeService runtimeService, TaskService taskService,
    ...
    public ResponseEntity<StartTourProcessResponse> startOrGetActiveTourProcessFromForm(
        ...
        return startOrGetActiveTourProcessFromRequest(new StartTourProcessRequest(restockerId, todayDeliveryCount));
    }
        ...
        try {
            synchronized (startLock) {
                ...
                List<ProcessInstance> activeProcesses = runtimeService.createProcessInstanceQuery()
                        .processDefinitionKey(PROCESS_DEFINITION_KEY).processInstanceBusinessKey(restockerId).active()
                        .list();
                ...
                if (!activeProcesses.isEmpty()) {
                    ...
                    runtimeService.setVariable(activeProcess.getProcessInstanceId(), "todayDeliveryCount",
                ...
                ProcessInstance processInstance = runtimeService.createProcessInstanceByKey(PROCESS_DEFINITION_KEY)
                        .businessKey(restockerId).setVariable("restockerId", restockerId)
            ...
            LOG.error("Could not start delivery process for restockerId={} with todayDeliveryCount={}", restockerId,
                    todayDeliveryCount, exception);
    ...
    private ResponseEntity<TaskLookupResponse> findTaskFromRequest(TaskLookupRequest request) {
        ...
        List<Task> tasks = taskService.createTaskQuery().processInstanceId(request.processInstanceId())
                .taskDefinitionKey(request.taskDefinitionKey()).list();
    ...
    public ResponseEntity<Void> completeTaskFromForm(@RequestParam("taskId") String taskId,
            @RequestParam(value = "variablesJson", required = false) String variablesJson)
            throws JsonProcessingException {
        Map<String, ProcessVariable> variables = isBlank(variablesJson) ? Map.of()
                : objectMapper.readValue(variablesJson,
                        objectMapper.getTypeFactory().constructMapType(Map.class, String.class, ProcessVariable.class));
